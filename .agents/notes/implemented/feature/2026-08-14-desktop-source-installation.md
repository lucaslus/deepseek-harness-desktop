...
The desktop app is distributed as source code. `apps/desktop/bootstrap.sh` shallow-clones the current `master` source into the fixed checkout at `~/.dsh-desktop`, then invokes `apps/desktop/install.sh`. The installer reuses an existing Node.js 24.15.0 runtime with Corepack when possible; otherwise it verifies and downloads that runtime into `apps/desktop/.runtime/node`. Its terminal UI presents animated high-level stages while command output is recorded in `~/Library/Logs/DeepSeek Harness Desktop/` and shown only when a stage fails. It uses Corepack to activate pnpm 11.7.0, installs the locked workspace dependencies, builds the repository, builds `DSH.app`, and opens it. It interactively offers a small real `/Applications/DeepSeek Harness.app` bundle rather than a symlink, records the source checkout in that bundle, registers it with macOS, and refreshes it after app updates. Node.js remains on the user's machine and is not copied into the app bundle.
...
**Require a globally installed Node.js or pnpm.** Global runtime and package-manager installations create avoidable setup dependencies and are unreliable for Finder launches. The installer opportunistically links an exact existing Node.js runtime inside the checkout, otherwise manages a verified copy there; its Corepack selects the repository-pinned pnpm version.
...
- Installation downloads the managed Node.js 24.15.0 runtime only when an exact local runtime is unavailable, and can take as long as a normal repository build; macOS Command Line Tools remain the one system-level prerequisite for Git and Swift.
