...
该命令会把仓库 clone 到 `~/.dsh-desktop`，将已校验的 Node.js 24.15.0 runtime 下载到该 checkout 内，通过其自带的 Corepack 选择 pnpm 11.7.0，构建并打开应用。安装器会询问是否将 **DeepSeek Harness** 加入 `/Applications`；选择 `y` 会创建符号链接而非复制一份 App，因此 Finder、Spotlight 和 Dock 始终打开仓库内当前构建的应用。
